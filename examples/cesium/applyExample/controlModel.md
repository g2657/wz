---
title: "Cesium 键盘控制飞行教程"
description: "详解 Cesium.js 键盘控制飞行：模型姿态控制对象，用于控制模型的偏航角(heading)、俯仰角(pitch)和翻滚角(roll)，涵盖 Viewer、Scene、Camera 等关键实现，附完整源码与在线 Demo，适合 Cesium 应用 学习与二次开发。"
head:
  - - meta
    - name: keywords
      content: "Cesium.js,键盘控制飞行,WebGL,源码,教程,在线案例,Cesium"
outline: deep
---

### 键盘控制飞行 · *controlModel* · [▶ 在线运行案例](https://z2586300277.github.io/three-cesium-examples/#/codeMirror?navigation=CesiumJS&classify=applyExample&id=controlModel)
...
本案例演示 **键盘控制飞行** 效果：模型姿态控制对象，用于控制模型的偏航角(heading)、俯仰角(pitch)和翻滚角(roll)。建议先打开文首在线案例查看动态画面，再对照下方源码逐步理解。
...
完整源码：[GitHub](https://github.com/z2586300277/three-cesium-examples/blob/dev/cesiumExamples/application/controlModel.js)

## 小结
- 本文提供 **键盘控制飞行** 完整 Cesium.js 源码与在线 Demo，建议先运行案例再改 uniform/参数做二次实验
